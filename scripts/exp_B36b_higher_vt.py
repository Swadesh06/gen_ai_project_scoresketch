"""B36b: extended vt range for PESTO+CREPE-voicing hybrid."""
...
def main(annotator: str = "A1") -> None:
    ...
    run = wandb.init(project="humscribe-v3.2", name=f"exp_B36b_higher_vt_{annotator}",
                     config={"git_sha": git_sha()},
                     tags=["B36b", "vocadito", "hybrid"], dir="logs/wandb")
    base = ModeConfig.for_mode("soft")
    rows = []
    for vt in (0.55, 0.60, 0.65, 0.70, 0.75, 0.80, 0.85, 0.90):
        for psw in (11, 15, 19):
            ...
            rows.append({"vt": vt, "psw": psw, "f1": mf, "p": mp, "r": mr})
            print(f"  vt={vt:.2f}  psw={psw}  F1={mf:.3f}  P={mp:.3f}  R={mr:.3f}")
            wandb.log({"vt": vt, "psw": psw, "f1": mf, "p": mp, "r": mr})
    rows.sort(key=lambda r: -r["f1"])
    print(f"\nTop 5:")
    for r in rows[:5]:
        print(f"  F1={r['f1']:.3f}  vt={r['vt']}  psw={r['psw']}")
    out = Path(f"reports/_exp_B36b_higher_vt_{annotator}.json")

"""Extended ME-14: 8 configs across tpb={6,8,12,16,18,24} + sanity on/off.

Builds on eval_me14_mv2h_ensemble.py. Goal: find if any other tpb beats 12.
"""
...
CONFIGS = [
    {"name": f"tpb{tpb}_sanity", "tpb": tpb, "octave_sanity": True}
    for tpb in (6, 8, 12, 16, 18, 24)
] + [
    {"name": "tpb12_no_sanity", "tpb": 12, "octave_sanity": False},
    {"name": "tpb24_no_sanity", "tpb": 24, "octave_sanity": False},
]
...
def main():
    rows = []
    for k in PIECES:
        scores = {c["name"]: _run_config(k, c) for c in CONFIGS}
        best_name = max(scores, key=lambda n: scores[n] if scores[n] is not None else -1)
        rows.append({"piece": k, **{n: s for n, s in scores.items()},
                      "best": best_name, "best_mv2h": scores[best_name]})
        print(f"{k:50s} " + " ".join(f"{c['name'][3:5]}:{(scores[c['name']] or 0):.3f}" for c in CONFIGS) + f"  best={best_name}")

    means = {n: float(np.nanmean([s for r in rows
                                     if (s:=r[n]) is not None and s > 0.01]))
              for n in scores}
    print("\nMean MV2H per config:")
    for n, v in sorted(means.items(), key=lambda x: -x[1]):
        print(f"  {n:25s} {v:.4f}")

    out = Path("reports/_exp_ME14_extended.json")

"""B84 — Bigger voice tracker (d=384, 10 layers) on same 237 ASAP data.

B76 used d=192, 6 layers (1.78M params) → 94.47% mean acc. B84 scales to
d=384, 10 layers (~12M params) to test if the voice tracking task has
more learnable structure than B76 captured. Same data, same training
regime, just more capacity.

Risk: overfit on 1384 train chunks. Use higher dropout (0.2) + cosine LR
to mitigate.
"""
...
OUT_JSON = Path("reports/_exp_B84_voice_transformer_big.json")
CHECKPOINT = Path("checkpoints/voice_transformer_b84/best.pt")
HELD_OUT = {
...
def load_piece(piece: str):
    ...
    try: mid = pretty_midi.PrettyMIDI(str(p))
    except Exception: return None
    if len(mid.instruments) < 2: return None
    notes, voices = [], []
    for vid, inst in enumerate(mid.instruments[:2]):
        for n in inst.notes:
            notes.append([float(n.pitch), float(n.start), float(n.end - n.start), float(n.start)])
            voices.append(vid)
...
class BigVoiceTransformer(nn.Module):
    def __init__(self, d_model=384, n_heads=8, n_layers=10, ff_dim=768, dropout=0.2):
        super().__init__()
...
def main(n_epochs: int = 80, d_model: int = 384, n_heads: int = 8,
         n_layers: int = 10, lr: float = 2e-4, chunk_size: int = 512) -> None:
    cfg_w = {"git_sha": git_sha(), "n_epochs": n_epochs, "d_model": d_model,
             "n_heads": n_heads, "n_layers": n_layers, "lr": lr, "chunk_size": chunk_size}
    run = wandb.init(project="humscribe-v3.2", name="exp_B84_voice_transformer_big",
                     config=cfg_w, tags=["B84", "asap", "voice-tracker", "big",
    ...
    model = BigVoiceTransformer(d_model=d_model, n_heads=n_heads, n_layers=n_layers).to("cuda")
    opt = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=5e-4)
    sched = torch.optim.lr_scheduler.CosineAnnealingLR(opt, T_max=n_epochs)
    ce = nn.CrossEntropyLoss()
    n_params = sum(p.numel() for p in model.parameters()) / 1e6
    print(f"  model: {n_params:.2f}M params")
    wandb.summary["n_params_M"] = n_params
    ...
    for epoch in range(n_epochs):
        ...
        with torch.no_grad():
            for piece, arr, vc in val_data:
                ...
                per_piece.setdefault(piece, []).append(float((pred == vc).mean()))

"""ASAP multi-piece with HMM voice tracker (Phase B Exp B25).
Same harness as exp_B12_asap_multi but uses voice_hmm instead of voice_tracking."""
...
def run_piece(piece_dir: Path, beat_tol: float = 0.07) -> dict:
    ...
    tpb = 24
    q_on, q_off = quantize_with_hmm_voice_tracking(notes, beats, tatums_per_beat=tpb)
    ...
    return {"piece": str(piece_dir.relative_to(ASAP)), "f_beat": f_beat,
            "stage5_raw": pct_raw, "stage5_snap": pct_snap,
            "n_pred_notes": int(len(notes)), "n_gt_notes": int(len(gt_iv)),
...
def main(n_pieces: int) -> None:
    ...
    cfg = {"exp": "B25_asap_multi_hmm_vt", "n_pieces": len(pieces), "git_sha": git_sha()}
    run = wandb.init(project="humscribe-v3.2", name=f"exp_B25_asap_multi_hmm_n{len(pieces)}",
                     config=cfg, tags=["B25", "asap", "hmm", "multi"], dir="logs/wandb")
    rows = []
    ...
    if rows:
        ...
        summary = {"n": len(rows),
                   ...
                   "stage5_pct_pass_90": float(np.mean([s >= 0.90 for s in snaps]))}
        wandb.summary.update(summary)
        print(f"\nSummary: mean snap={summary['mean_stage5_snap']:.3f}  raw={summary['mean_stage5_raw']:.3f}")
        print(f"  pct >= 0.85: {summary['stage5_pct_pass_85']*100:.0f}%  >= 0.90: {summary['stage5_pct_pass_90']*100:.0f}%")
    out = Path("reports/_exp_B25_asap_multi_hmm.json")
